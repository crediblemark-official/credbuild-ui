...
export const CustomEmbedRender = ({ content, styling }: CustomEmbedProps) => {
    ...
    if (isStandalone) {
        ...
        if (processedHtml.includes("</body>")) {
            processedHtml = processedHtml.replace("</body>", `${resizeScript}</body>`);
        } else {
            processedHtml = processedHtml + resizeScript;
        }

        // Inject Tailwind CDN secara dinamis hanya jika kode memang menggunakan kelas Tailwind
        if (needsTailwind && !processedHtml.includes("cdn.tailwindcss.com") && !processedHtml.includes("tailwindcss")) {
            if (processedHtml.includes("</head>")) {
                processedHtml = processedHtml.replace("</head>", `<script src="https://cdn.tailwindcss.com"></script></head>`);
            } else if (processedHtml.includes("<head>")) {
                processedHtml = processedHtml.replace("<head>", `<head><script src="https://cdn.tailwindcss.com"></script>`);
            } else {
                processedHtml = `<script src="https://cdn.tailwindcss.com"></script>` + processedHtml;
            }
        }
